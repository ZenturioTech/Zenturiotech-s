    ...
    <button
      ...
      onClick={() => {
        ...
        if (pathname !== path) {
          ...
          // Step 1: Move glider to parent button (Services or Who we are)
          const parentBtn = path.startsWith('/services') 
            ? document.querySelector('.nav-link-dropdown-btn')
            : document.querySelector('.nav-link-who-we-are-btn');
          const dock = dockRef.current;
          if (parentBtn && dock) {
            const dockRect = dock.getBoundingClientRect();
            const selRect = parentBtn.getBoundingClientRect();
...
export default function Header() {
  ...
  const [mobileWhoWeAreDropdownOpen, setMobileWhoWeAreDropdownOpen] = useState(false);
  const servicesDropdownRef = useRef<HTMLDivElement>(null);
  const mobileServicesDropdownRef = useRef<HTMLDivElement>(null);
  const whoWeAreDropdownRef = useRef<HTMLDivElement>(null);
  const mobileWhoWeAreDropdownRef = useRef<HTMLDivElement>(null);
  const [keepDropdownOpen, setKeepDropdownOpen] = useState(false);
  const [keepWhoWeAreDropdownOpen, setKeepWhoWeAreDropdownOpen] = useState(false);
  ...
  useEffect(() => {
    const handleScroll = () => {
      ...
      if (currentScrollY <= 0) {
        ...
        setKeepWhoWeAreDropdownOpen(false);
      } else {
  ...
  useEffect(() => {
    ...
    if (servicesDropdownOpen || whoWeAreDropdownOpen) {
      document.addEventListener('mousedown', handleClickOutside);
    } else {
      document.removeEventListener('mousedown', handleClickOutside);
    }
    return () => document.removeEventListener('mousedown', handleClickOutside);
  }, [servicesDropdownOpen, whoWeAreDropdownOpen]);
  ...
  useEffect(() => {
    function handleMobileClickOutside(event: MouseEvent) {
      ...
      if (mobileWhoWeAreDropdownRef.current && !mobileWhoWeAreDropdownRef.current.contains(event.target as Node)) {
        setMobileWhoWeAreDropdownOpen(false);
      }
    }
    if (mobileServicesDropdownOpen || mobileWhoWeAreDropdownOpen) {
      document.addEventListener('mousedown', handleMobileClickOutside);
    } else {
      document.removeEventListener('mousedown', handleMobileClickOutside);
    }
    return () => document.removeEventListener('mousedown', handleMobileClickOutside);
  }, [mobileServicesDropdownOpen, mobileWhoWeAreDropdownOpen]);
  ...
  return (
    <>
      ...
      <Flex as="header" align="left" padding="16" style={{ position: 'fixed', top: 0, left: 0, width: '100vw', zIndex: 100, background: 'transparent', backdropFilter: 'none', height: 110, pointerEvents: 'none' }}>
        ...
        >
          ...
          <div
            ...
            onMouseEnter={() => !keepWhoWeAreDropdownOpen && setWhoWeAreDropdownOpen(true)}
            onMouseLeave={() => !keepWhoWeAreDropdownOpen && setWhoWeAreDropdownOpen(false)}
          >
            <button
              type="button"
              className={`nav-link nav-link-flex nav-link-who-we-are-btn${pathname === '/who-we-are' || pathname === '/sereinsoul' ? ' selected' : ''}`}
              ...
              onClick={() => {
                if (pathname !== '/who-we-are') {
                  ...
                  setKeepWhoWeAreDropdownOpen(true);
                  setWhoWeAreDropdownOpen(true);
                }
              }}
              aria-haspopup="true"
              aria-expanded={whoWeAreDropdownOpen}
            >
            ...
            {whoWeAreDropdownOpen && (
              ...
              >
                <DropdownItem 
                  label="SereinSoul" 
                  path="/sereinsoul" 
                  closeDesktopDropdown={() => setWhoWeAreDropdownOpen(false)}
                  closeMobileDropdown={() => setMobileWhoWeAreDropdownOpen(false)}
                />
              </div>
            )}
          </div>
      ...
      <Flex as="nav" gap="32" align="center" className="nav-dock-mobile" ref={mobileDockRef}>
        ...
        <div style={{ position: 'relative' }} ref={mobileWhoWeAreDropdownRef}>
          <button
            type="button"
            aria-label="Who we are"
            className={`nav-link-mobile${pathname === '/who-we-are' || pathname === '/sereinsoul' ? ' selected' : ''}`}
            style={{ 
              background: 'none', 
              border: 'none', 
            ...
            onClick={() => {
              if (pathname !== '/who-we-are' && pathname !== '/sereinsoul') {
                if (mobileNavTimeoutRef.current) clearTimeout(mobileNavTimeoutRef.current);
                const currentEl = document.querySelector(`.nav-link-mobile[data-path='${pathname}']`);
                const dock = mobileDockRef.current;
                if (currentEl && dock) {
                ...
                setMobileWhoWeAreDropdownOpen(v => !v);
              }
            }}
            aria-haspopup="true"
            aria-expanded={mobileWhoWeAreDropdownOpen}
            data-path="/who-we-are"
          >
            <HiOutlineUserGroup style={{ fontSize: 28 }} />
          </button>
          {mobileWhoWeAreDropdownOpen && (
            <div
        ...
        <Link href="/about" aria-label="Contact Us" className={`nav-link-mobile${pathname === '/about' ? ' selected' : ''}`} data-path="/about"
          onClick={e => {
            if (pathname !== '/about') {
              e.preventDefault();
              if (mobileNavTimeoutRef.current) clearTimeout(mobileNavTimeoutRef.current);
      ...
      <style>{`
        ...
        @media (max-width: 1070px) {
          ...
          .nav-dock-mobile a, .nav-dock-mobile button {
            color: #fff;
            display: flex;
            align-items: center;
        ...
        .nav-link:hover, .nav-link-mobile:hover, .nav-link:focus, .nav-link-mobile:focus {
        ...
        .nav-link-who-we-are-btn:focus, .nav-link-who-we-are-btn:hover {
          color: #2ea6ff;
        }
